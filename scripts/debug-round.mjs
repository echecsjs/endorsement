...
for (const [w, b] of missing) {
  const ourW = dryResult.pairings.find((p) => p.white === w || p.black === w);
  const ourB = dryResult.pairings.find((p) => p.white === b || p.black === b);
...
console.log('\nbracket entries:');
const bracketEnters = events.filter((e) => e.type === 'dutch:bracket-enter');
...
console.log('\ncolor allocation:');
const colorEvents = events.filter((e) => e.type === 'pairing:color-allocated');
