#!/usr/bin/env bash
#
# generates N tournaments with JaVaFo RTG, checks each with our FPC.
#
# usage: ./scripts/check-javafo.sh [count] [javafo-jar-path]
...
# vary tournament sizes — JaVaFo config format: key=value
configs=(
  "PlayersNumber=40\nRoundsNumber=9"
  "PlayersNumber=60\nRoundsNumber=11"
  "PlayersNumber=80\nRoundsNumber=9"
...
for i in $(seq 0 $((N - 1))); do
  ...
  # JaVaFo RTG: -g config -o output
  if ! java -ea -jar "$JAVAFO" -g "$config_path" -o "$trf_path" -w 2>/dev/null; then
    echo "seed $seed ($cfg_label): JaVaFo RTG failed"
    crashes=$((crashes + 1))
    rm -f "$config_path"
    continue
  fi
  ...
  if [ "$rounds_perfect" = "$rounds_total" ]; then
    ...
    if [ -n "${FAILURES_DIR:-}" ]; then
      ...
      cp "$trf_path" "$FAILURES_DIR/javafo_seed_${seed}_${cfg_players}p_${cfg_rounds}r.trf"
    fi
  fi
...
echo ""
echo "=== JaVaFo RTG -> our FPC: $N seeds (start: $START_SEED) ==="
